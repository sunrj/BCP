...
void Graph::subOPT_Swap(I d, I sL, I sR, I swap)
{
    ...
    if (swap)
    {
        for (auto u : cand_L[d])
        {
            ...
            cand_L[d+1] = cand_L[d] * p_adj[u];
            excl_L[d+1] = excl_L[d] * p_adj[u];
            cand_R[d+1] = cand_R[d] - p_adj[u];
            excl_R[d+1] = excl_R[d] - p_adj[u];

            VI temp_ans_L(ans_L.begin(), ans_L.begin()+sL+1);
            VI temp_ans_R(ans_R.begin(), ans_R.begin()+sR);
            ...
            for (auto v : cand_L[d+1])
            {
                ...
                {
                    I flag = 1;
                    VI tmp(temp_ans_R - n_adj[v]);
                    if (tmp.size() > K) flag = 0;
                    else
                    {
                        for (auto w : tmp)
                            if (sL+1-negD[w] == K) {flag = 0; break;}
                    }
                    if (flag) temp_save.push_back(v);
                }
            }
            cand_L[d+1] = temp_save;
            temp_save.clear();
            ...
            {
                I flag = 1;
                VI tmp(temp_ans_L - n_adj[v]);
                if (tmp.size() > K) flag = 0;
                else
                {
                    for (auto w : tmp)
                        if (sR-negD[w] == K) {flag = 0; break;}
                }
                if (flag) temp_save.push_back(v);
            }
        ...
        for (auto u : cand_R[d])
        {
            ...
            for (auto v : n_adj[u]) 
                if (select_L[v]) negD[v]++;

            cand_R[d+1] = cand_R[d] * p_adj[u];
            excl_R[d+1] = excl_R[d] * p_adj[u];
            excl_L[d+1] = excl_L[d] - p_adj[u];

            VI temp_ans_L(ans_L.begin(), ans_L.begin()+sL);
            VI temp_ans_R(ans_R.begin(), ans_R.begin()+sR+1);
