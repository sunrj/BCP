...
void Graph::OPT()
{
    ...
    for (auto u : U)
    {
        ...
        for (auto v : part_L)
        {
            ...
            for (auto w : p_adj[v])
                if (select_L[w]) p_deg[v]++;
            for (auto w : n_adj[v])
            {
                if (select_R[w]) n_deg[v]++;
        ...
        for (auto v : part_R)
        {
            p_deg[v] = 0; n_deg[v] = 0;
            for (auto w : p_adj[v])
                if (select_R[w]) p_deg[v]++;
            for (auto w : n_adj[v])
                if (select_L[w]) n_deg[v]++;
        }
        ...
        for (auto v : part_L)
            if (select_L[v])
                if (p_deg[v] < R-1 || n_deg[v] < R-K || cn_deg[v] < R-2*K)
                {
                    ...
                    Lpart_size--;
                    if (v == u) {flag = 0; break;}
                    Pruning_PratL(v, u, flag, Lpart_size, Rpart_size);
                    if (!flag) break;
                }
        if (flag)
        {
                ...
                if (select_R[v])
                    if (p_deg[v] < R-1 || n_deg[v] < R-K)
                    {
                        if (select_R[v] == 2)
                        {
                            select_R[v] = 0;
                            Rpart_size--;
                            Pruning_PratR(v, u, true, flag, Lpart_size, Rpart_size);
                            if (!flag) break;
                        }
                        else
                        {
                            select_R[v] = 0;
                            Rpart_size--;
                            Pruning_PratR(v, u, false, flag, Lpart_size, Rpart_size);
                            if (!flag) break;
                        }
                    }
        }
...
void Graph::Pruning_PratL(I v, I &u, I &flag, I &Lpart_size, I &Rpart_size)
{
    if (!flag) return;
    for (auto w : p_adj[v])
        if (select_L[w])
        {
            ...
            if (p_deg[w] < R-1)
            {
                ...
                Pruning_PratL(w, u, flag, Lpart_size, Rpart_size);
                if (!flag) return;
            }
        }
    for (auto w : n_adj[v])
        if (select_R[w])
        {
            ...
            if (n_deg[w] < R-K)
            {
                if (select_R[w] == 2)
                {
                    select_R[w] = 0;
                    Rpart_size--;
                    Pruning_PratR(w, u, true, flag, Lpart_size, Rpart_size);
                    if (!flag) return;
                }
                else
                {
                    select_R[w] = 0;
                    Rpart_size--;
                    Pruning_PratR(w, u, false, flag, Lpart_size, Rpart_size);
                    if (!flag) return;
                }
            }
        }
...
void Graph::Pruning_PratR(I v, I &u, bool use_cn, I &flag, I &Lpart_size, I &Rpart_size)
{
    if (!flag) return;
    for (auto w : p_adj[v])
        if (select_R[w])
